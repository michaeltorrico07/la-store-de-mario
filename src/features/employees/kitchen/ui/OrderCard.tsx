...
export const OrderCard = ({ order, onUnitConfirm, onTotalConfirm }: OrderCardProps) => {
  ...
  const isUrgent = () => {
    const now = new Date();
    const deliveryTime = new Date(order.deliveryTime); // asumiendo que es Date

    const timeDiff = deliveryTime.getDate() - now.getTime();

    // Retorna true si la entrega es en los próximos 15 minutos (y no pasó aún)
    ...
    <tr className={`border-2 ${getStatusColor(order.status)} transition-all duration-300 hover:shadow-md`}>
      ...
      <td className="px-4 py-6">
        <div className="flex flex-col">
          ...
          <div className="text-xs text-gray-500">Pedido: {order.id}</div>
        </div>
      </td>
      ...
      <td className="px-4 py-6 text-center">
        <div className={`flex flex-col items-center ${isUrgent() ? 'text-red-600' : 'text-gray-800'}`}>
          ...
          <span className="text-xl font-bold">{
            order.deliveryTime
              ? new Date(order.deliveryTime).toLocaleString('es-AR', {
                day: '2-digit',
                month: '2-digit',
                year: 'numeric',
                hour: '2-digit',
